...
class Twitter extends TwitterAbstract
{
    ...
    protected function extractProfileCard()
    {
        ...
        $localetemp = $profileCard->find('.ProfileHeaderCard-locationText')[0];
        if (empty($localetemp)) {
            $localetemp = $profileCard->find('.ProfileHeaderCard-location a')[0];
        }
        ...
        $account = Account::create(
            ...
            'locale' => $this->sanitizeNodeText($localetemp),
            'website' => $this->sanitizeNodeAttr($profileCard->find('.ProfileHeaderCard-url a')[0]),
            'dateOfBirth' => $profileCard->find('.ProfileHeaderCard-birthdateText span')[0],
